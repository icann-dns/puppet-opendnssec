...
class opendnssec (
  ...
  String[1,32]          $group                  = $::opendnssec::params::group,
  ...
  Opendnssec::Datastore $datastore_engine       = $::opendnssec::params::datastore_engine,
  ...
  if $manage_packages {
    ...
    file {[$base_dir, "${base_dir}/signed", "${base_dir}/unsigned"]:
      ensure => 'directory',
      mode   => '0640',
      owner  => $user,
      group  => $group;
